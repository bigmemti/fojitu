        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form action="{{route('course.update',['course' => $course])}}" method="POST" class="flex flex-col gap-4">
                        @csrf
                        @method('patch')
                        <label for="name" class="text-xl font-bold">name:</label>
                        <input type="text" name="name" id="name" class="dark:bg-gray-900 dark:text-white" value="{{$course->name}}">
                        @error('name')
                            <span>{{$message}}</span>
                        @enderror
