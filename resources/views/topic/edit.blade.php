        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form action="{{route('topic.update',['topic' => $topic])}}" method="POST" class="flex flex-col gap-4">
                        @csrf
                        @method('patch')
                        <label for="name" class="text-xl font-semibold">name:</label>
                        <input type="text" name="name" id="name" value="{{old('name', $topic->name)}}" class="dark:bg-gray-900">
                        @error('name')
                            <span>{{$message}}</span>
                        @enderror
